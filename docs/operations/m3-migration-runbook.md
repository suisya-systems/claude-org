...
- The 3-step `claude-org` ⇄ `claude-org-en` swap is present and ordered: `claude-org → claude-org-tmp`, then `claude-org-en → claude-org`, then `claude-org-tmp → claude-org`.
...
- The `_research/_runs/{ccswarm,anthropic,claude-org-audit,_solo}` cluster is materialized via `ensure_dir`.
...
`--from-manifest` replays the supplied operations exactly; it does NOT regenerate the plan from inventory. The `workers_root` and `archive_quarter` baked into the manifest are honored (so `--workers-root` here is ignored). Preflight (cross-drive / source-missing / target-conflict / **root-escape** / **active-runs** when `--db` is also passed) still runs against the loaded ops. Use `--manifest` to choose where the executed-op log is written (the migrator persists it incrementally after every successful op so a mid-batch failure still leaves a valid rollback record).
...
4. **Remaining run moves** under `claude-org/`, `claude-org/`, `core-harness/`, `renga/`, `claude-org-runtime/`.
...
for proj in renga claude-org claude-org claude-org-runtime core-harness; do
  git -C ../workers/$proj worktree list
done
# 2. each worktree healthy
for wt in $(git -C ../workers/claude-org worktree list --porcelain | awk '/^worktree/{print $2}'); do
...
```
mv ../workers/claude-org          ../workers/claude-org-tmp
mv ../workers/claude-org-en       ../workers/claude-org
mv ../workers/claude-org-tmp   ../workers/claude-org
```
...
| `claude-org-tmp/` left behind after a failed swap | apply died between step 1 and step 3 of the 3-step rename | run `--rollback --manifest <step-N-manifest>`; the manifest was written incrementally so the in-progress entry IS recorded. If rollback is unavailable, manually `mv ../workers/claude-org-tmp ../workers/claude-org` (no other dir should be claiming that name during a stalled swap), then re-run `git -C ../workers/claude-org worktree repair`. |
---
