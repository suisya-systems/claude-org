...
to the Lead (Set F § 4 T5 / T7 / T8 / T9); the tool only surfaces
...
without Lead cleanup, or a `spawn_claude_pane` failure that the
dispatcher did not report back.

**Why warn-only.** `runs.status='queued' → 'abandoned'` (T8) is a
**Lead-owned** transition that the contract currently flags as
*prescribed but not yet implemented* (§ 4 T8). Auto-healing here would
both pre-empt the Lead's outcome judgment and write a transition
...
2. If the reservation should be released, confirm with the Lead
   ...
   manually via the Lead skill flow.
...
**Why warn-only.** The Lead owns the outcome classification (Set
...
state — no Lead classification is owed. The remediation is a pure
