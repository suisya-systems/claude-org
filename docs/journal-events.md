# Journal Event Catalog (claude-org)

> **Storage update (M4 / Issue #267).**
...
> claude-org emits via `tools/journal_append.{sh,py}` (DB-routed
...
`send_message` and the dispatcher / Lead persists the event.
...
- **Emitted by** — the role(s) whose action *originates* the event
  ...
  written by the Lead on receipt of the corresponding peer
...
| `retro_deferred`         | `worker`, `reason`                                          | dispatcher   | dispatcher | —            | Retro Steps 1–2 could not be completed before `close_pane` (e.g., Lead unreachable within 5 minutes); pane close skipped. Listed at Set B §1 aborted row as a visible journal event, but no §2 transition's mandatory-Journal line cites it. |
