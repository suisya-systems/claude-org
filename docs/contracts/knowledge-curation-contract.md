...
> - `.claude/skills/org-curate/references/knowledge-standards.md` (record format `Facts / Judgment / Evidence / Applicable Situations`, merge / promotion criteria)
...
- **Format**: Markdown. Body conforms to the four-heading record format in `.claude/skills/org-curate/references/knowledge-standards.md`: `## Facts`, `## Judgment`, `## Evidence`, `## Applicable Situations`.
...
- **Format**: Markdown. Body conforms to `org-curate` Step 3: `# {theme name}` H1, then per-knowledge `## {learning title}` H2 sections that synthesize the four-heading content from the underlying raw entries.
...
- **Format**: Markdown. Per-candidate blocks delimited by `### {YYYY-MM-DD} {pattern-name}` H3 headings; bullet fields per the entry-format block in the file itself (`decision score`, `matching signals`, `evidence`, `related tasks`, `related raw files`, `caller`, `proposed skill name`, `status`, `decision date`, `rejection reason`, `merged into`).
- **Schema**: `status` ∈ `{pending, approved, rejected, merged-into-{existing-skill}}`. Once an entry leaves `pending`, it is retained as history (not deleted) per the file's "Operational notes" section.
- **Owner**: `skill-eligibility-check` Step 4 (auto-append on `skill_recommend` decisions). Status transitions (`pending` → `approved` / `rejected` / `merged-into-*`, plus `decision date` / `rejection reason` / `merged into` fields) are written by the worker that performs the corresponding promotion task (per the §2.4 execution-path decision). The curator does NOT directly hand-edit this file; its writes happen through `skill-eligibility-check`. The secretary does NOT edit this file in place; for human-relayed batch decisions, the secretary delegates the status update to a worker alongside the skill creation.
...
- **Lifecycle**: append-only at the entry level. Same `pattern_name` while still `pending`: existing entry's `related tasks` / `related raw files` are merged in (no new entry). Same `pattern_name` after a terminal status (`approved` / `rejected` / `merged-into-*`): a new dated entry is added so the prior decision and rationale survive as history.
...
- **Trigger**: a worker finishes a task in `validation_depth: full` and has a reusable, non-obvious learning (per Set A § Role: worker and `knowledge-standards.md` "recording criteria") **or** the dispatcher completes `org-retro` Step 1–3 with a reusable process learning.
...
- **Trigger**: `skill-candidates.md` `pending` count crosses N=5 (per `skill-audit` Step 1 and the Issue #68 batch-question rationale). Lead batch-prompts the human. On human approval, the secretary delegates skill creation (`.claude/skills/{name}/SKILL.md` from `.claude/skills/org-retro/references/work-skill-template.md`) and the corresponding `skill-candidates.md` status update to a worker via `org-delegate`, per the §2.4 execution-path decision below.
- **Effect**: new skill file under `.claude/skills/{name}/`; `skill-candidates.md` entry transitions to `approved` (with `decision date`) or `rejected` (with `rejection reason`). `merged-into-{existing-skill}` is used when the candidate's value is folded into an existing skill rather than creating a new one.
- **Constraint**: terminal-status entries are retained as history, not deleted (per the file's "Operational notes").
...
- **MAY write (delegated promotion task only)**: `.claude/skills/{name}/SKILL.md` (and surrounding files under `.claude/skills/{name}/`) AND the corresponding `knowledge/skill-candidates.md` status transition (the entry's `status` / `decision date` / `rejection reason` / `merged into` fields), but ONLY when the worker's brief explicitly delegates a §2.4 skill-promotion task. Outside such a brief, these paths remain forbidden. Set A § Role: worker reconciled this carve-out into its `Outputs > Files written` list (issue #251).
