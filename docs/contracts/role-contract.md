...
- **Permission mode**: Not subject to `default_permission_mode`. Runs with the Claude Code default (per-tool prompts) because it is the human-judgment surface (`registry/org-config.md` § "Per-role scope of application", Issue #10).
...
- **Hands off all real work to workers** — must not edit code, run tests, build, debug, or `git commit` substantive changes itself. When a problem is reported, must not investigate locally; it goes back to a worker (`CLAUDE.md` § "Role boundaries").
- **Communication discipline** — no jargon to the human (e.g., "PR #12" → "We submitted the login feature changes"). Must offer choices when a request is ambiguous.
...
- **Permission mode**: Hard-coded `bypassPermissions` regardless of `default_permission_mode`. Reason: spawning child Claude agents is classified as "Create Unsafe Agents" by `auto`, which would block worker spawn. (`registry/org-config.md`; `org-start` SKILL "Dispatcher".)
- **Model**: `sonnet` (cost optimization). Acceptable here only because the dispatcher does not run through the safety classifier.
- **No human dialogue** — the dispatcher never talks to the human directly (`.dispatcher/CLAUDE.md` "Does not directly converse with humans.").
- **All replies to secretary use stable name `to_id="secretary"`**; numeric `from_id` may be used only as a fallback when `to_id="secretary"` returns `[pane_not_found]` (per `.dispatcher/CLAUDE.md` § "How to reply to the Lead").
- **Reports are routed to secretary, not back to dispatcher** — the dispatcher must explicitly tell each worker on spawn: "Report to the Lead. Not to the Dispatcher."
...
- **Optional** — manual invocation via secretary (e.g., "organize the knowledge"). Without dispatcher-provided reasons, `org-curate` Step 0 runs the threshold script itself.
...
- **No human dialogue** — `.curator/CLAUDE.md` "Does not directly converse with humans." Communication only via secretary.
...
- **Codex discipline** (full mode, when `codex` available):
  - Diff self-review uses the **review surface**: `codex exec review --base main -m gpt-5.5 -c model_reasoning_effort=medium` (pass the branch's base branch to `--base`) (approximately 2x faster than the old direct `codex exec` heavy-prompt form on small/medium diffs, at parity on safety-critical findings). The `codex:rescue` skill remains forbidden (past 18-min hangs). Operational guards and the speed/coverage caveats are the SoT of `knowledge/curated/codex.md`.
  - Pre-implementation **design review** has no diff, so it keeps the `codex exec` prompt form (multi-perspective), which retains broader coverage of subtle / design-level findings; the same model / effort and the `codex:rescue` prohibition apply.
  - The model is effectively pinned to `gpt-5.5` on the ChatGPT account (`gpt-5.5-codex` and the API-key surface are not executable).
