> **This document is historical reference material.** Do not consult it for Lead / worker / dispatcher standard operations. The standard path is CI / merge-watch monitoring via [`/pr-watch-pane <PR>`](../../.claude/skills/pr-watch-pane/SKILL.md) (which internally runs `tools/pr-watch.sh --merge-watch` in a dedicated pane), or `python tools/run_complete_on_merge.py --pr <PR>` (Issue #317). If `tools/run_complete_on_merge.py` behaves unexpectedly, do not reproduce it manually; file an Issue and pause the affected close until the bug on the resolver / helper side is fixed. Any decision to do manual work as an exception is left to the user's explicit judgment. If Lead reaches this document on its own, that is a protocol violation.
