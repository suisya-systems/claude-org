...
1. Snapshot `mcp__renga-peers__list_panes` and pick balanced-split target / direction. The narrative rationale (why rect-based selection, role priority, the `[split_refused]` failure mode, the verification trace at `secretary 280×43 / dispatcher 140×43 / curator 140×43`) lives in [`.claude/skills/org-delegate/references/pane-layout.md`](../.claude/skills/org-delegate/references/pane-layout.md). The deterministic SoT pane-layout.md points at — `claude-org-runtime dispatcher delegate-plan` / `claude_org_runtime.dispatcher.runner.choose_split()` plus the `MIN_PANE_*` / `SECRETARY_MIN_*` / `_ROLE_PRIORITY` constants — was retired in PR #139, and pane-layout.md explicitly removed the prose copy of those constants (Issue #307 cleanup). Until pane-layout.md is updated or the runtime helper is restored, pick the split heuristically from the `list_panes` rect data, treating pane-layout.md's documented floors `MIN_PANE_WIDTH = 20` / `MIN_PANE_HEIGHT = 5` (line 39 of that file) as the rejection threshold for `[split_refused]`. Flag this gap to the Lead if a delegation hits an ambiguous split; do not invoke any retired CLI or library.
