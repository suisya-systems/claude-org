...
- The canonical path the Secretary uses to refresh the Dispatcher session when its context grows long (Issue #464) (the `mcp__renga-peers__*` calls in the steps below are written in the operational-frame / renga wording; when `ORG_TRANSPORT=broker`, per the opening "Transport (two-track)" section, the fully qualified names are mechanically rewritten to `mcp__org-broker__*` — argument shapes and semantics are identical):
