...
## Step 0.5: herdr backend compatibility preflight (broker+herdr only; pre-spawn fatal gate, Issue #748)

> **Positional invariant (do not confuse with Block C2)**: this gate runs **after all five sub-steps of Step 0 are complete, and before Block A fires the dispatcher `spawn_claude_pane`**, and it is a **synchronously executed fatal gate**. Block C2 (runtime drift) is a warning that runs in parallel with Block A's spawn and is responsible for "org-start continues even when unconfirmed"; this gate is the exact opposite — **on exit 1/2 it stops without firing Block A's spawn**. Its purpose is to detect, before the dispatcher spawn and with a fail-loud, a silent out-of-window promotion caused by herdr's self-update (the daemon promoting past the runtime's supported protocol window), and thereby prevent the `agent.start` wedge caused by a protocol mismatch (the main symptom of Issue #151, whose cause gets misdiagnosed as "the daemon is stopped"). Making this a parallel warning alongside C2 would re-introduce the incident, so it must synchronously block **before** the spawn.

The value of `ORG_TRANSPORT` alone does not decide whether this fires (there is a renga/broker frame difference, and even under broker the backend is not necessarily herdr). The gate reads the **resolved backend** that the runtime writes into the `daemon.json` sidecar under `$ORG_BROKER_STATE_DIR` (`.state/broker` when unset), and fires **only** when `backend == "herdr"`. Non-broker (sidecar absent) or a non-herdr backend (tmux / wezterm / null) passes straight through as a skip (exit 0).

> **Host execution is required (inside the sandbox it misjudges)**: `herdr --version` / the herdr socket ping / the daemon sidecar all depend on host reality (PATH / process namespace / Unix domain socket). Running inside Claude Code's Bash sandbox produces false negatives/positives from PATH and namespace differences. As with Block C2, **run it on the host by attaching `dangerouslyDisableSandbox: true`**.

1. Run the compatibility preflight **on the host** (with the Claude Code Bash tool, attach `dangerouslyDisableSandbox: true`):
   ```bash
   py -3 tools/check_herdr_compat.py       # Windows
   python3 tools/check_herdr_compat.py     # Mac/Linux
   ```
2. **Branch on the exit code** (do not mix this exit contract with C2's drift warning):
   - **exit 0** (compatible or skip): compatibility confirmed, or not applicable because of a non-broker / non-herdr backend. **Proceed as-is to the parallel startup of Steps 1-3.**
   - **exit 1** (incompatible fatal): the resolved herdr wire protocol is outside the runtime's `SUPPORTED_PROTOCOLS` window. **Stop without firing Block A's spawn** and **present stdout's diagnostic block verbatim to the user** (state_dir / daemon backend / installed herdr / derived protocol / daemon ping / runtime supported / effective protocol / Failures / Remediation). Remediation is a choice of two: "raise the runtime" or "lower herdr". Have the user resolve the mismatch one way or the other, then re-run /org-start.
   - **exit 2** (unverified fatal): the backend is herdr but the protocol cannot be determined (socket ping unreachable **and** `herdr --version` unobtainable / runtime import failure, etc.). Letting an unverified result continue as a warning would re-introduce the incident, so **stop without firing the spawn, exactly as for exit 1**, and present the diagnostic to the user (the cause is most often running inside the sandbox). Prompt a host re-run with `dangerouslyDisableSandbox: true`.

> Design notes:
> - **Determination layers (in order of authority)**: (1) **socket ping (primary, authoritative)** — send one `ping` round-trip to the daemon's herdr socket to obtain the protocol number the daemon is **actually speaking** (the runtime adapter settles it with the same ping). (2) **`herdr --version` + local map (fallback)** — the alternative when the socket is unreachable. Derives the protocol from the PATH binary's version via a local map (`0.7.0-0.7.1 -> 14 / 0.7.2-0.7.4 -> 16 / 0.7.5 and later -> 17`). However, the `herdr` binary on PATH is not necessarily the one the daemon is connected to, and this path cannot detect a protocol bump within a version series, so the fact that the fallback was used is **stated explicitly on stderr**.
> - **The SoT of the supported window is the runtime**: the supported protocol window is not hard-coded on the ja/EN side; it is cross-checked by importing `claude_org_runtime.terminal.herdr.SUPPORTED_PROTOCOLS` (if a compat API is added to the runtime in the future, prefer it; otherwise this fallback). Even as the window widens with each runtime release, the description here does not go stale.
> - **stdout is the diagnostic block / JSON only**; limitation notes and diagnostics go to **stderr** (`--json` also emits machine-readable output). The exit code is the only branching key.
> - Script body: [`tools/check_herdr_compat.py`](../../../tools/check_herdr_compat.py).

## Steps 1-3: parallel startup phase

> **Issue #410 / Stage B**: As soon as Step 0 (all five sub-steps: transport determination / set_summary / MCP connectivity / identity verification / workers_dir verification) is complete **and Step 0.5's herdr compatibility preflight has passed with exit 0 (compatible / skip)**, fire the `spawn_claude_pane` for the dispatcher, and in parallel with waiting for Claude to boot (~30-60s), run Block B (DB read of previous state) / Block C (dashboard server startup). The goal is to compress wall-clock time from ~3 minutes (when run serially) down to ~35s. **If Step 0.5 exits 1/2, do not fire Block A** (a synchronous fatal gate that prevents the spawn wedge).
>
...
2. **Branch on the exit code** (stdout is the drift line only; diagnostics go to stderr):
   ...
   - **exit 2** (unconfirmed): latest could not be determined — PyPI unreachable / abnormal response / outside the pin window / missing `packaging`, **or the installed distribution is a local install (`file://` path / VCS / editable, determined via PEP 610 `direct_url.json`) which cannot be compared against PyPI**. A reason diagnostic is printed to stderr. Because **drift is unverified**, state "runtime drift: PyPI unconfirmed (re-run on host required)" explicitly in the Step 4 report (do not go silent). **A local install is not resolved by re-running on the host** (the installed artifact is not guaranteed to correspond to a release), so when the stderr diagnostic shows `local install -- PyPI 照合不能`, reword it as "runtime: cannot be compared against PyPI because it is a local install (needs re-checking with a PyPI-sourced install)" when reporting. For every other case (offline / inside the sandbox), re-run with `dangerouslyDisableSandbox: true`.
...
> - **local install detection (Issue #747)**: when the installed distribution came from a `file://` path / VCS checkout / editable install, its version is not guaranteed to correspond to a release (a local build can claim any version it likes). This is determined via PEP 610 `direct_url.json` (read through `importlib.metadata.Distribution.read_text`, not by globbing dist-info directly), and when it applies the check falls to **exit 2 + a stderr diagnostic (`local install -- PyPI 照合不能`) before reaching PyPI**. This is the root fix for the incident where a `file://` install was misreported as "up to date, no drift". Non-local direct URLs such as https archives are left to the normal PyPI comparison.
...
**Handling Block C2's runtime drift output**: Block C2 returns its result via **exit code** (stdout is the drift line only; diagnostics go to stderr). On **exit 1** (drift), transcribe stdout's `[runtime drift] ...` line, for any of the templates below, **verbatim at the end, separated by one blank line**. **exit 0** (PyPI reachability confirmed, no drift) attaches no warning line. **exit 2** (PyPI unconfirmed = offline / inside the sandbox / abnormal response / outside the pin window / missing `packaging`, **or the installed distribution is a local install (`file://` / VCS / editable) that cannot be compared against PyPI**) is **not silent** — attach a one-line "runtime drift: PyPI unconfirmed (re-run on host required)" to the report (to prevent the silent misread of #119). **However, when the stderr diagnostic shows `local install -- PyPI 照合不能`, a host re-run does not resolve it**, so attach it reworded as "runtime: cannot be compared against PyPI because it is a local install (needs re-checking with a PyPI-sourced install)" (#747: the root fix for the incident where a `file://` install was misreported as "up to date, no drift"). **exit 3** (runtime not installed) attaches "runtime not installed (needs checking)". Running inside the sandbox tends to produce exit 2, so run Block C2 on the host with `dangerouslyDisableSandbox: true`.
