...
- **In renga environments, pr-watch sends a peer message to the Secretary at the moment of CI completion / merge detection / 24h timeout** (Issue #326). The Lead does not poll the events table; it advances to the next step on arrival of `<channel source="renga-peers"> CI_COMPLETED: PR #<n> ...` (and `PR_MERGED: PR #<n>` / `PR_MERGE_WATCH_TIMEOUT: PR #<n>` / `PR_MERGED_NO_RUN: PR #<n>`). `CI_COMPLETED` received -> ask the user for merge approval -> user approval -> `PR_MERGED` received -> go to 2b-ii post-merge cleanup. `PR_MERGED_NO_RUN` is a failure case where merge was observed but no corresponding run row was found (the `no_run` terminal state of `tools/run_complete_on_merge.py`); do not proceed to post-merge cleanup — handle by human judgment. In plain shells / CI without `RENGA_SOCKET` set, peer-send is a silent noop and falls back to polling the events table as before.
  - **Under broker (`ORG_TRANSPORT=broker`)**: the `CI_COMPLETED` / `PR_MERGED` / `PR_MERGE_WATCH_TIMEOUT` / `PR_MERGED_NO_RUN` sent by pr-watch arrive **push-primary** under broker (the per-pane channel sidecar `server:org-broker-channel` injects into the Lead's idle session via `notifications/claude/channel`; runtime push-first 0.1.24+, transport-lab `docs/design/broker-native-roles.md` §9; the channel source is `org-broker`). The semantics / branching of the message body are identical to renga. **On push failure the fallback** is the Lead actively pulling the body with `mcp__org-broker__check_messages` at the start of each turn before advancing to the next step (a nudge can be a trigger, but it does not wake an idle session, so an active poll is the canonical path — §9.6). In addition, as with RENGA_SOCKET, when the broker daemon is not running (plain shell / CI), peer/channel delivery itself does not stand up, so it falls back to polling the events table as before.
