...
`tools/pr-watch.sh` / `tools/pr-watch.ps1` / `tools/pr_watch.py` open `state.db` via a relative path, so if the cwd at launch is not the ja root, writing the CI-completion event will crash and peer notifications (`CI_COMPLETED` / `PR_MERGED` etc.) will not fire. The canonical route [`/pr-watch-pane`](../pr-watch-pane/SKILL.md) deterministically resolves the ja-root absolute path and spawns the pane, so it absorbs the cwd trap (this cwd resolution is one of the pane route's main purposes). This caution applies only to the emergency fallback where a human manually launches `tools/pr-watch.sh` via `!`; in that case, always use the form `cd <ja-root> && bash tools/pr-watch.sh <PR> ...`. A root fix (cwd-independence) is in progress in Issue #398.

### Warning: run monitoring in a dedicated pane (do not make Bash background launch canonical)

The canonical route for the Lead to start CI monitoring is [`/pr-watch-pane <PR>`](../pr-watch-pane/SKILL.md), not the route of invoking `tools/pr-watch.*` directly via a Claude Code Bash tool background launch. A Claude Code background task tracks only the spawning shell, and the self-detached monitoring body (including `nohup ... &` + `disown`) is orphaned (official spec), so in a fresh session right after `/clear` / [`/secretary-resume`](../secretary-resume/SKILL.md) or at session end the monitoring dies silently along with it, and neither the CI-completion event nor the peer notification fires at all (the process disappears quietly and only an empty log file is left, which is hard to notice). **A Bash wrapper's completion notification (with exit code) does not mean the monitoring body completed**: even after the wrapper returns, the orphaned monitoring may have died silently, so do not treat the completion notification as a signal of CI-monitoring completion — always confirm via the `ci_completed` row in the events table. The dedicated-pane route runs outside the sandbox and independently of the Lead session, so it structurally avoids this orphaning.

### Event-driven close of the watcher pane (`pr-watch-<PR>`) by the Lead at each monitoring terminus (root fix for the herdr zombie residue, Issue #751)

The watcher pane (`name="pr-watch-<PR>"`) spawned by [`/pr-watch-pane`](../pr-watch-pane/SKILL.md)
**self-closes** via the trailing `tmux kill-pane` when the monitoring body (`tools/pr-watch.sh`)
exits. That self-close, however, is **a low-latency path that only works on a tmux backend**: in an
environment where broker runs on a **herdr / wezterm backend**, `tmux kill-pane` becomes a no-op and
**the watcher pane remains as a zombie** (observed: on 2026-07-22 three watchers for PR #154 / #749 /
#750 remained on the herdr backend. For the backend-dependence details see the "Prerequisites"
section of [`/pr-watch-pane`](../pr-watch-pane/SKILL.md)).

For that reason, **the canonical path is for the Lead to clean up the watcher pane event-driven at
each monitoring terminus** (the tmux self-close is preserved as the low-latency path; to keep double
cleanup harmless, treat `[pane_not_found]` as a normal response):

- **Terminal triggers (any of them) and cleanup timing**: at every terminus the watcher body
  (`tools/pr-watch.sh`) has **already exited by the time that terminal event is emitted**, so
  **clean up the pane immediately on receipt of the terminal event** (independently of the run.status
  transition gate or the human-confirmation gate. Delaying cleanup to a later step leaves a zombie on
  a herdr backend for the entire wait). The termini: `PR_MERGED` (-> then the 2b-ii post-merge
  cleanup) / `PR_MERGE_WATCH_TIMEOUT` (-> clean up immediately on receipt, below) / a CI-failure
  decision (-> clean up immediately at the entrance to the 2c feedback loop) /
  `PR_MERGED_HEAD_UNCONFIRMED` and `PR_MERGED_NO_RUN` (-> **clean up immediately without waiting for
  human confirmation**, leaving only the run-completion judgment at the human-confirmation gate).
- **Bind the cleanup target to the watcher instance noted at spawn time (do not re-derive it by
  `name`)**: terminal events (`PR_MERGED` / a CI-failure decision etc.) carry the **PR number and head
  SHA** in the body but **not the pane_id**. Meanwhile a watcher gets a **new pane_id** on every
  restart even for the same PR (a CI failure -> re-push spawns a new `pr-watch-<PR>`, etc.). If
  cleanup **re-derives** the live pane by `name="pr-watch-<PR>"`, an **old** terminal event delivered
  late / duplicated resolves the **already-restarted new watcher**, and closing that numeric pane_id
  **wrongly closes the replacement monitor** (converting name -> id does not escape the same trap). To
  avoid this, the Lead **keeps the pane_id noted when `/pr-watch-pane` was started (Step 3's "Spawned
  pane id=N") and the monitored head as the watcher instance's identity, and binds cleanup to that
  identity**:
  - **freshness gate (the root fix for wrong closes)**: confirm that **the watcher's monitored head**
    indicated by the terminal event (= the pushed SHA that watcher was tracking CI for) **matches the
    monitored head of the watcher instance currently being tracked**. Note that **the field the
    monitored head is read from differs by event kind**:
    - the CI-completion family (`ci_completed` / `CI_COMPLETED`) / `PR_MERGE_WATCH_TIMEOUT` /
      `PR_MERGED` / `PR_MERGED_NO_RUN`: the body's `head` (the same ground truth as the events-DB
      `head` match; see the "CI-completion detection, the canonical path" section).
    - **`PR_MERGED_HEAD_UNCONFIRMED`: the body's `head` is the newly merged SHA and differs from the
      monitored head**, so match the `last CI-confirmed head` (baseline) field as the monitored head
      instead (deciding on `head` always mismatches, so cleanup would be wrongly skipped as
      superseded and a zombie would stay on herdr for the whole human-confirmation wait - avoid this
      mix-up).
    Treat only old / duplicate events whose monitored head does not match as superseded and do not
    close them (= do not kill an already-restarted new watcher). On a match, close as below. If the
    tracking is already gone (that instance was already cleaned up), it is a no-op.
  - **identifier-bound close**: on a match, verify with `mcp__renga-peers__list_panes` that the noted **numeric
    pane_id** still points at `name="pr-watch-<PR>"` and `role="watcher"` (an identity check against
    pane_id recycling; if it has been reassigned to another pane, do not close it), and when it
    agrees, close with `mcp__renga-peers__close_pane(target=<the noted pane_id>)` and clear the tracking.
    `[pane_not_found]` / `[pane_vanished]` are the normal responses for already self-closed on tmux /
    already cleaned up on herdr, and are skipped.
  - **use a name target only for a stale registry binding**: only when the noted pane_id is already
    gone **and** a same-named spawn is rejected with `[name_taken]`, use a **name target**
    `mcp__renga-peers__close_pane(target="pr-watch-<PR>")` to pop the registry (there is no live pane in this case,
    so there is no room for a wrong close).
  - `close_pane` covers **both tmux and herdr** on the transport abstraction. The SoT for the case
    split is [`/pr-watch-pane`](../pr-watch-pane/SKILL.md) Step 5.
- **A different operation from the worker pane's `CLOSE_PANE`**: closing the worker Claude pane in
  2b-ii is requested from the dispatcher with `CLOSE_PANE: {pane_id}`, but the watcher pane is a CLI
  pane the Lead spawned, so the Lead cleans it up directly with `close_pane` (the Lead holds the
  `spawn_pane` ops tier; contract Surface 8).

### On receiving `PR_MERGE_WATCH_TIMEOUT` (a monitoring terminus with the merge unconfirmed, Issue #751)

When `pr-watch --merge-watch` cannot observe a merge even 24h after CI went green and emits
`PR_MERGE_WATCH_TIMEOUT`, **monitoring terminates with the merge unconfirmed** (do not proceed to the
2b-ii post-merge cleanup - no merge has happened). The Lead does the following:

- **Clean up the watcher pane immediately**: use the procedure of the "Event-driven close of the
  watcher pane ... by the Lead at each monitoring terminus" section above (bind to the pane_id noted
  at spawn time plus the head freshness gate and close after the identity check; do not re-derive by
  `name`). This is required because self-close does not take effect on a herdr backend and the pane
  would remain (on a tmux backend `[pane_not_found]` = already self-closed, which is normal).
- **Do not touch run.status**: because the merge is unconfirmed, leave it at REVIEW and do not call
  `update_run_status(..., 'completed')` (the 2b-ii close conditions are not met). Present to the user
  whether the merge is merely delayed or has stalled, and if monitoring is to resume, restart
  [`/pr-watch-pane <PR>`](../pr-watch-pane/SKILL.md) (cleanup is done, so the spawn is idempotent).
...
- **Clean up the watcher pane (`pr-watch-<PR>`) immediately, without waiting for human confirmation (Issue #751)**: the watcher body has already exited at this point, so clean it up independently of the human-confirmation gate, following the procedure of the "Event-driven close of the watcher pane ... by the Lead at each monitoring terminus" section above (delaying cleanup until human confirmation leaves a zombie on a herdr backend for the entire confirmation wait). **In the freshness gate, match `last CI-confirmed head` (baseline) as the monitored head, not this event's `head` (the merge SHA)** (deciding on `head` always mismatches and wrongly skips cleanup). Leave only the run-completion judgment to the human-confirmation gate below.
...
- **When monitoring terminates on a CI-failure decision, the Lead cleans up the watcher pane (`pr-watch-<PR>`) (Issue #751)**: on a CI-failure decision (`ci_completed` with `status='failed'`) `tools/pr-watch.sh` terminates monitoring (even with `--merge-watch` it exits without waiting for the merge). **On a herdr / wezterm backend self-close does not take effect and the watcher pane remains**, so the Lead cleans it up before entering the fix instructions (bind to the pane_id noted at spawn time plus the head freshness gate and close after the identity check; do not re-derive by `name`. For details see the "Event-driven close of the watcher pane (`pr-watch-<PR>`) by the Lead at each monitoring terminus" section above). If it has already self-closed on a tmux backend you get `[pane_not_found]` (normal: already self-closed, fine to skip). On the re-push after the fix, start [`/pr-watch-pane <PR>`](../pr-watch-pane/SKILL.md) again and re-track its new pane_id / head (duplicate delivery of the old event is judged superseded on the head mismatch and does not wrongly close the new watcher).
- **Release the dispatcher's monitoring flag before the re-instruction (Issue #658, T6 monitoring-resume contract)**: If the worker has already reported completion (at completion, the secretary sent `WORKER_COMPLETION_NOTED` to the dispatcher per §2a, so `completion_reported_at` is set), then **before** sending the additional instruction, send `WORKER_REOPENED` to the dispatcher **best-effort and non-blocking**, clearing `completion_reported_at` to `null`. Because the re-instruction is a direct secretary→worker send with the dispatcher off the path, this explicit release is the **fast-path release** for the dispatcher's PANE_OUTPUT_WITHOUT_PEER_MSG detector ([`.dispatcher/references/worker-monitoring.md`](../../../.dispatcher/references/worker-monitoring.md) Step 5.2 — maintained in the upstream ja repository, not mirrored here; see CONTRIBUTING "Mirror scope"). Do not wait for a dispatcher reply (the dispatcher reflects it on its normal `/loop 3m` `check_messages`). Without the release the skip stays sticky and misses a real silent dead-lock during the awaiting_review→in_progress review fix. **Even if `WORKER_REOPENED` is dropped, the `run.status='in_use'` transition immediately below (written deterministically by StateWriter) acts as a reliable backstop: the dispatcher observes `runs.status == 'in_use'` and self-heal-resumes monitoring** (so the release does not depend on the best-effort notice alone and does not fail unsafe, Issue #658 P2). Include task_id and reopened_at (ISO-8601 UTC) in the body:
  ```
...
- Ask the dispatcher to close the pane (**the worker Claude pane**):
  `CLOSE_PANE: please close pane {pane_id}.`
- **The Lead cleans up the CI-monitoring watcher pane (`pr-watch-<PR>`) (Issue #751)**: separately from the worker pane, the Lead cleans up the watcher pane spawned by `/pr-watch-pane` (bind to the pane_id noted at spawn time plus the head freshness gate and close after the identity check; do not re-derive by `name`. The SoT for the procedure is the "Event-driven close of the watcher pane (`pr-watch-<PR>`) by the Lead at each monitoring terminus" section above). `close_pane` covers **both tmux and herdr** on the transport abstraction, and it is required in post-merge cleanup because **self-close does not take effect on a herdr / wezterm backend and the pane remains**. If it has already self-closed on a tmux backend you get `[pane_not_found]` (normal: already self-closed, fine to skip).
- **Post-processing per directory pattern** (do at the same time):
  ...
  - Pattern B (worktree): run `git -C {workers_dir}/{project_slug}/ worktree remove --force .worktrees/{task_id}`. Keep the branch (do not delete the branch even after merge, for PR history).
    ...
    - **For self-edit (`pattern_variant='live_repo_worktree'`)**: the worktree base is `{claude_org_path}`, so run `git -C {claude_org_path} worktree remove --force .worktrees/{task_id}` (Issue #289). The reason for `--force` is the same as normal Pattern B. Keep the branch likewise.
  ...
  - **Special cleanup for Pattern C (`gitignored_repo_root`, claude-org self-edit) (Issue #478)**: `worker_dir` is the claude-org repo root itself, so neither worktree remove nor dir removal applies, and `{claude_org_root}/CLAUDE.local.md` (the Worker-instruction brief) lingers. If it remains, on the next `/org-start` the Lead loads a contradictory "Lead and Worker" role identity. **At close, call `cleanup_pattern_c_local_md()` from `tools/run_complete_on_merge.py` to delete the brief** (bundled into the StateWriter block below). Detection is `runs.pattern == 'C'` AND `worker_dir == claude_org_root`, and it is a no-op for ephemeral C / Patterns A and B. One `pattern_c_cleanup` row (payload: `task` / `removed_path` / `mode`) is appended to `events`. Idempotent (`mode=skip` when the file is absent). **Issue #486**: when the `remove_worker_dir()` in the block below DELETEs the `worker_dirs` row, `runs.worker_dir_id` becomes `ON DELETE SET NULL` and the join-based `worker_dir` resolution turns NULL, making the cleanup a no-op; pass the deleted abs path explicitly via `worker_dir_abs=` to make it order-independent. When a PR-driven close calls `tools/run_complete_on_merge.py --pr <PR>`, the same cleanup runs automatically at merge-record time, but gitignored tasks rarely produce a PR, so the explicit call from the StateWriter block below is the primary route. `.claude/settings.local.json` is out of scope (a separate Issue) because it needs worker-origin vs. Lead-origin discrimination.
...
Once the 2b-ii post-merge cleanup (run -> COMPLETED, pane close, worktree post-processing) is done, the Lead proactively presents next-work candidates to the user without waiting to be asked. **Candidate generation does not improvise `gh issue list` on the spot; it consumes the [`/work-discovery`](../work-discovery/SKILL.md) skill (= the triage output of the deterministic tool `tools/work_discovery_scan.py`).** This codifies the decision criteria (dependencies resolved / priority / effort) and gives the presentation reproducibility and auditability. The primary policy reference is [`CLAUDE.md`](../../../CLAUDE.md) "Next-task proposals after a PR merge (proactive next-dispatch)"; the primary design reference is [`docs/design/work-discovery-triage.md`](../../../docs/design/work-discovery-triage.md) §8 (post-merge integration).

- **Run with the post_merge trigger**: launch `/work-discovery` in the post-merge context (scan with `--trigger post_merge`, and `--free-panes <n>` if there are free panes). The candidate JSON carries `generated_for: "post_merge"`, and **in post-merge the `unblocked_by_recent_merge` axis — which surfaces "Issues unblocked by the most recent merge / natural follow-ups" near the top — has the strongest effect** (design §4.2 / §8.1-3). If there are free slots, the rank of `parallelizable` candidates is raised so the free slots are filled.
- **Presentation and the human gate stay compatible with current behavior**: the Lead renders the triage result in design §5.2 format (N candidates + 1 recommendation, estimated axes marked `(estimated)`, with an excluded slot shown) for the human. **The human picks by number → the chosen candidate enters the normal delegation flow from Step 0 of [`/org-delegate`](../org-delegate/SKILL.md)**. Only the means of candidate generation changes from improvisation to triage; the human's operations and the outer shape of the human gate stay the same (design §8.1-4).
- **Stop in propose-only**: once the candidates are presented, stop. Do not auto-start rank 1 (the recommendation), auto-commit, or auto-PR (only the human decides what to start; design INV-1 / INV-2). This step and `/work-discovery` are also forbidden from calling org-delegate or spawning.
- For manual / on-demand presentation (idle time, etc.), launch the same `/work-discovery` with `--trigger manual`. The exit-code branches and rendering rules within the skill are documented as the primary reference in [`/work-discovery`](../work-discovery/SKILL.md).
