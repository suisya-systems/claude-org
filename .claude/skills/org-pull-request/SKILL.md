...
> **Transport layer both systems (`ORG_TRANSPORT`: default `renga` / opt-in `broker`)**: this skill's `mcp__renga-peers__*` calls (fix-instruction `send_message` to the worker, etc.) and pr-watch's peer notifications are written for **default `renga`** and can be followed as-is when `ORG_TRANSPORT` is unset (default behavior unchanged). Under `ORG_TRANSPORT=broker` (opt-in, revertible), the fully qualified names get machine-substituted to **`mcp__renga-peers__*` → `mcp__org-broker__*`**, and **the receipt of CI_COMPLETED / PR_MERGED etc. becomes a pane-local nudge + `check_messages` pull rather than an in-band push** (see the receive-model note in 2b-i below). Errors gain the broker-specific codes (see the broker section in [`.claude/skills/org-delegate/references/renga-error-codes.md`](../org-delegate/references/renga-error-codes.md)). See CLAUDE.md "Transport layer (transport) both systems" and [`docs/contracts/backend-interface-contract.md`](../../../docs/contracts/backend-interface-contract.md) Surface 8 (awaiting ratification) for details. The default-renga procedure is unchanged (broker is additive).
...
- **In renga environments, pr-watch sends a peer message to the Secretary at the moment of CI completion / merge detection / 24h timeout** (Issue #326). The Lead does not poll the events table; it advances to the next step on arrival of `<channel source="renga-peers"> CI_COMPLETED: PR #<n> ...` (and `PR_MERGED: PR #<n>` / `PR_MERGE_WATCH_TIMEOUT: PR #<n>` / `PR_MERGED_NO_RUN: PR #<n>`). `CI_COMPLETED` received -> ask the user for merge approval -> user approval -> `PR_MERGED` received -> go to 2b-ii post-merge cleanup. `PR_MERGED_NO_RUN` is a failure case where merge was observed but no corresponding run row was found (the `no_run` terminal state of `tools/run_complete_on_merge.py`); do not proceed to post-merge cleanup — handle by human judgment. In plain shells / CI without `RENGA_SOCKET` set, peer-send is a silent noop and falls back to polling the events table as before.
  - **Under broker (`ORG_TRANSPORT=broker`)**: the `CI_COMPLETED` / `PR_MERGED` / `PR_MERGE_WATCH_TIMEOUT` / `PR_MERGED_NO_RUN` sent by pr-watch arrive not as an in-band `<channel source="renga-peers">` push but as a **pane-local nudge** (the channel source is `org-broker`). The Lead, **on seeing the nudge, pulls the body via `mcp__org-broker__check_messages`** before advancing to the next step (the semantics / branching of the message body are identical to renga; only the receive trigger becomes pull). Like with RENGA_SOCKET, when the broker daemon is not running (plain shell / CI), it falls back to polling the events table as before.
...
- **At the merge-approval presentation, re-present the human-comprehension summary as well (verification depth `full` only)**: When CI is green and asking the user for merge approval, re-present the summary that was already persisted in [`.claude/skills/org-delegate/SKILL.md`](../org-delegate/SKILL.md) Step 5 (2a) into the Progress Log of `.state/workers/worker-{task_id}.md` under a `Human Understanding Summary:` heading + fenced code block (use the latest block if there are multiple completions), so the user can make the final merge decision without opening the diff. If no persisted copy exists (e.g., PRs that were in-flight before this format was introduced), read the summary that remains in the PR description / worker completion-report message (the Lead does not reconstruct it by reading the diff itself). The schema SoT is [`.claude/skills/org-delegate/references/worker-claude-template.md`](../org-delegate/references/worker-claude-template.md). Not attached for minimal tasks
