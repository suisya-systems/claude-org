...
> **Transport — both rails (`ORG_TRANSPORT`: default `broker` / opt-in `renga`)**: this skill's `mcp__org-broker__*` (such as `send_message` for fix instructions to the worker) is written in the **code-default `broker`** frame (`tools/transport.py: DEFAULT_TRANSPORT`, the migration target; see the "two-frame note" below first), and the tool names can be followed as-is. However, **the route by which `CI_COMPLETED` / `PR_MERGED` etc. arrive as broker push first** (the per-pane co-located channel sidecar `server:org-broker-channel` injects the body into the Lead's idle session via `notifications/claude/channel`. runtime push-first 0.1.24+. The design SoT is transport-lab `docs/design/broker-native-roles.md` §9) **applies only when `ORG_TRANSPORT=broker` is explicit**: the pr-watch peer-notification helper `tools/peer_notify.py: notify_peer` falls back to renga when it is unset (the operational default), so reception in an unset environment is the renga in-band push (see the two-frame note below and 2b-i). **Fallback when push is lost depends on the sending transport** (only when `ORG_TRANSPORT=broker` is explicit does the Lead actively `mcp__org-broker__check_messages` at the start of a turn; when unset and routed via renga, the message is not on the broker queue and is received by polling the events table. See the receive-model note in 2b-i below. §9.6). Errors gain broker-extended codes (see the broker section in [`.claude/skills/org-delegate/references/renga-error-codes.md`](../org-delegate/references/renga-error-codes.md)). With `ORG_TRANSPORT=renga` (opt-in, revertible), fully qualified names are mechanically replaced from **`mcp__org-broker__*` -> `mcp__renga-peers__*`** and CI_COMPLETED etc. arrive as `<channel source="renga-peers">` in-band push (the procedure is the same shape; renga is not removed and remains as an always-available revert safety net). For details see the "Transport (both rails)" section in CLAUDE.md and [`docs/contracts/backend-interface-contract.md`](../../../docs/contracts/backend-interface-contract.md) Surface 8 (ratified 2026-06-14. The additive S3 revision to push first is ratified (2026-06-15); the existing ratified body is unchanged).

> **Two-frame note on "default" (Refs #604)**: when this skill writes "default `broker`" with `mcp__org-broker__*` as its primary, that is the **code default** frame (`tools/transport.py: DEFAULT_TRANSPORT` was flipped to `broker` in runtime 0.1.28 / Epic #586; migration target). **The operational default is renga** (broker live-run dogfood is not yet active until Epic #6 Issue G), the same two-frame relationship as in the "Transport (both rails)" section of the root `CLAUDE.md`. **Important (actual behavior of pr-watch peer notification)**: the peer-notification helper `tools/peer_notify.py: notify_peer` that pr-watch uses to send to the Secretary on CI completion / merge / timeout does **not** look at `DEFAULT_TRANSPORT`; it uses **raw env judgment**, going through the broker CLI (`claude-org-runtime broker send`) path only when `ORG_TRANSPORT==broker` is explicit, and **when unset (operational default) falls back to renga / `RENGA_SOCKET`** (silent no-op if `RENGA_SOCKET` is also unset). Therefore in the operationally default (unset) environment, `CI_COMPLETED` etc. arrive as the **renga in-band `<channel source="renga-peers">` push**; the broker channel sidecar's push first is the reception route only when `ORG_TRANSPORT=broker` is explicit (2b-i below describes both rails).
...
- **pr-watch sends a peer message to the Secretary at the moment of CI completion / merge detection / 24h timeout** (Issue #326). When `ORG_TRANSPORT=broker` is explicit, `CI_COMPLETED` / `PR_MERGED` / `PR_MERGE_WATCH_TIMEOUT` / `PR_MERGED_NO_RUN` arrive as **the broker channel sidecar's push first** (the per-pane co-located channel sidecar `server:org-broker-channel` injects the body into the Lead's idle session via `notifications/claude/channel`. runtime push-first 0.1.24+. transport-lab `docs/design/broker-native-roles.md` §9. channel source is `org-broker`). When unset (the operational default), `notify_peer` falls back to renga, so the messages arrive as `<channel source="renga-peers">` in-band push just like the `ORG_TRANSPORT=renga` branch below. The Lead does not poll the events table; it advances to the next step on arrival of `CI_COMPLETED: PR #<n> ...` (and `PR_MERGED: PR #<n>` / `PR_MERGE_WATCH_TIMEOUT: PR #<n>` / `PR_MERGED_NO_RUN: PR #<n>`). `CI_COMPLETED` received -> ask the user for merge approval -> user approval -> `PR_MERGED` received -> go to 2b-ii post-merge cleanup. `PR_MERGED_NO_RUN` is a failure case where merge was observed but no corresponding run row was found (the `no_run` terminal state of `tools/run_complete_on_merge.py`); do not proceed to post-merge cleanup — handle by human judgment. **Fallback when push is lost is matched to the transport actually used**: only when `ORG_TRANSPORT=broker` is explicit does the Lead actively `mcp__org-broker__check_messages` at the start of a turn to pull the body from the broker queue (a nudge can be a trigger but does not wake an idle session, so the active poll is the primary route. §9.6). Notifications routed via renga when unset (operational default) are not on the broker queue and cannot be picked up by `mcp__org-broker__check_messages` — in that case the `<channel source="renga-peers">` in-band push is primary, and any miss is caught by polling the events table below. Under either transport, in plain shells / CI without the broker daemon running / `RENGA_SOCKET` unset, peer / channel delivery itself does not work, so polling the events table is the final fallback as before.
  - **With `ORG_TRANSPORT=renga` (opt-in)**: pr-watch in-band pushes `<channel source="renga-peers"> CI_COMPLETED: PR #<n> ...` (and `PR_MERGED: PR #<n>` / `PR_MERGE_WATCH_TIMEOUT: PR #<n>` / `PR_MERGED_NO_RUN: PR #<n>`) at the same moment. The message-body semantics and branches are identical to broker. In plain shells / CI without `RENGA_SOCKET` set, peer-send is a silent noop and falls back to polling the events table as before.
