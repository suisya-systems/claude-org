...
5. If the human picks "shelve it for now" (status `deferred`, Issue #753):
   - Updating the `knowledge/skill-candidates.md` entry's status from `pending` to `deferred` goes through worker delegation (`org-delegate`); the Lead / Dispatcher does not edit it directly (per Set E §1.4 owner definition).
