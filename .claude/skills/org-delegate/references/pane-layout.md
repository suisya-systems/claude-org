...
> **Transport layer (transport) both systems — default `renga` / opt-in `broker`**: this file's `mcp__renga-peers__*` calls are written for **default `renga`** (`ORG_TRANSPORT` unset) and can be followed as-is (default behavior unchanged). Under `ORG_TRANSPORT=broker` (opt-in, revertible) the MCP server name becomes `org-broker`, and tools' **fully qualified names get machine-substituted from `mcp__renga-peers__*` → `mcp__org-broker__*`** (argument shape and semantics are identical, so balanced-split judgment and operation logic are unchanged). Only the transport-dependent points are noted in broker form:
>
> - **Spawn rite (folder-trust approval + dev-channel sidecar approval re-introduced)**: as described under "Why use `spawn_claude_pane`" below, renga injects `--dangerously-load-development-channels server:renga-peers`. Broker injects `--mcp-config <broker>` and at startup machine-approves Claude Code's **folder-trust prompt** via `send_keys(enter=true)` **in addition to** loading the channel sidecar with `--dangerously-load-development-channels server:org-broker-channel` for push-primary and **re-introducing** the dev-channel approval prompt (spawn-flow 3-3b) for machine approval (additive, not a replacement, of the ratified §5/§8.5 folder-trust flow; design transport-lab `docs/design/broker-native-roles.md` §9.5). Note that the broker spawn helper internally constructs the interactive-TUI argv behind the default-deny billing-neutral guard.
> - **Receive model (push-primary = `claude/channel` / pull fallback)**: renga's `send_message` pushes in-band. Broker has been **redesigned to push-primary** (runtime push-first 0.1.24+, broker-native-roles.md §9): the per-pane channel sidecar (`server:org-broker-channel`) injects the body into the idle session via `notifications/claude/channel`. Pull (pane-local nudge + `check_messages`) is the fallback layer, where reception is via each role's active cadence poll as the canonical path (a nudge is non-dependent because it does not wake an idle session; the §9.6 reading-substitution table). The layout operations themselves (`list_panes` / `spawn_claude_pane` / `close_pane` etc.) follow the same logic with only the tool name changing to `mcp__org-broker__*`.
> - **Error branching (broker additional codes)**: on top of renga codes such as `split_capacity_exceeded` / `[split_refused]` / `[cwd_invalid]`, broker may return `[token_invalid]` / `[session_invalid]` / `[tool_not_authorized]` / `[no_backend]` (= adapter_unavailable) / `[nudge_failed]` / `[peer_not_found]` / `[name_taken]`. See the broker section in [`.claude/skills/org-delegate/references/renga-error-codes.md`](renga-error-codes.md).
>
> `new_tab` / `focus_pane` are **absent** from the broker surface (intentional exclusion) — but the balanced split in this document uses only intra-tab splits and not `new_tab`, so the absence has no impact under broker. The canonical contract is [`docs/contracts/backend-interface-contract.md`](../../../../docs/contracts/backend-interface-contract.md) Surface 8 (ratified 2026-06-14; the push-primary additive amendment S3 is ratified 2026-06-15, with existing ratified text unchanged); the design SoT is transport-lab `docs/design/broker-native-roles.md` §9 (push-primary redesign) / `docs/design/ja-migration-plan.md` §5.2(ii). Broker real-run (dogfood) is scoped to Epic #6 Issue G and is not this file's default path.
