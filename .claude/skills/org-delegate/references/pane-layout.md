...
>
> - **Spawn rite (dev-channel approval → folder-trust approval)**: as described under "Why use `spawn_claude_pane`" below, renga injects `--dangerously-load-development-channels server:renga-peers`. Broker injects `--mcp-config <broker>` instead, and the startup approval prompt shifts to the Claude Code **folder-trust prompt** (machine-approval via `send_keys(enter=true)` keeps the same shape). Note that the broker spawn helper internally constructs the interactive-TUI argv behind the default-deny billing-neutral guard.
> - **Receive model (push → pull)**: renga's `send_message` pushes in-band; broker fires a pane-local nudge plus a `check_messages` pull. The layout operations themselves (`list_panes` / `spawn_claude_pane` / `close_pane` etc.) follow the same logic with only the tool name changing to `mcp__org-broker__*`.
...
>
> `new_tab` / `focus_pane` are **absent** from the broker surface (intentional exclusion) — but the balanced split in this document uses only intra-tab splits and not `new_tab`, so the absence has no impact under broker. The canonical contract is [`docs/contracts/backend-interface-contract.md`](../../../../docs/contracts/backend-interface-contract.md) Surface 8 (proposed, awaiting ratification); the design SoT is transport-lab `docs/design/ja-migration-plan.md` §5.2(ii). Broker real-run (dogfood) is scoped to Epic #6 Issue G and is not this file's default path.
