...
on disk. **Active Work Items remain the dispatcher's T2 responsibility** and are
not touched here.

---

## 1. Pattern selection (Step 0.7 + Step 1)

### Step 0.7 — gitignored target check (highest precedence)

The resolver checks each `--target` with `git -C <project_path> check-ignore -q --`.
If any target is ignored, the dispatch is forced into **Pattern C — gitignored
submode** (`pattern_variant='gitignored_repo_root'`):
...
local git repo. When skipped, the normal Pattern A/B decision runs.
...
When Step 0.7 does not force gitignored submode, the resolver reads
...
**Pattern B submode — `live_repo_worktree` (Issue #289)**: when the role
resolves to `claude-org-self-edit` (that is, the project is claude-org itself and
mode is `edit`), the resolver automatically substitutes the worktree base with
...
writes that status before any pane spawns. Without that, two back-to-back
delegations on the same project would both choose Pattern A and collide on the
base clone.

`A` reuse vs new clone is not a Pattern split — both share the same
`worker_dir` shape. Stage 3 `apply` checks the filesystem at execution time
...
## 2. Role detection (Step 1.5 — "How to choose the role")
...
write hooks the worker does not need (Codex Design Review M-4).
...
Pattern C `gitignored_repo_root` also forces the brief filename to
...
"writes a self-edit-style brief" from "is the `claude-org-self-edit` role".
...
(`tests/fixtures/delegate_payload/`) lock this format down so the historical
"verification_depth row dropped" failures remain impossible.
...
historically introduced by relying on the legacy path (settings env mismatch,
