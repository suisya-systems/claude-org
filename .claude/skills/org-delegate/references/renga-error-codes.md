...
| `nudge_failed` | The pull (fallback) nudge could not be delivered to the destination pane | `send_message` | The body is queued but the receiver cannot notice via nudge. Under push-primary the body is delivered by the channel sidecar; this code applies to the fallback pull path. Retry or escalate to Lead |
...
> **Broker's ok-return / delivery difference**: the renga exceptions described below (the renga-unreachable ok-text shim for `list_peers` / `send_message`) **do not exist in broker** (§6.3 carve-out does not apply). Under broker, transport loss is also returned as an error code per §8.7. The receive model has been **redesigned to push-primary (`claude/channel`) / pull fallback** (runtime push-first 0.1.24+, transport-lab `docs/design/broker-native-roles.md` §9): the per-pane channel sidecar (`server:org-broker-channel`) injects the body into idle via `notifications/claude/channel`, and when the sidecar is absent / unhealthy / channel-incapable it degrades to the pull fallback. The fallback's receive trigger is each role's active cadence poll (`mcp__org-broker__check_messages`) as the canonical path; the daemon's pane-local nudge can be a trigger when it fires, but it is not depended on since it does not wake an idle session (the existing "see the nudge → check_messages" prose is not retracted and is read as this fallback cadence — §9.6). `nudge_failed` is the code for **fallback-pull nudge** delivery failure; under the push-primary path, body delivery is handled by the channel sidecar. Contract: Surface 8 / §8.4 / §8.8 has **push-primary ratified** (2026-06-15, S3; §8.4 to push-primary; §8.8 to `receive_mode="push"`; supersedes the broker pull-only and pull is retained as fallback; details in [`docs/contracts/backend-interface-contract.md`](../../../../docs/contracts/backend-interface-contract.md); existing ratified text unchanged).
