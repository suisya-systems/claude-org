...
- **full** (new feature implementation / fix / refactor / test addition / hook, skill, or config edits, and anything else involving code or behavior changes)
  ...
  - **Additional gate (optional)**: after completing the commit, if the `codex` CLI is available, run a Codex diff self-review with **`codex exec review` (review surface)** (the long-prompt direct `codex exec` form is deprecated; on small/medium diffs the review surface is roughly 2x faster and at parity for safety-critical Blocker/Major findings)
    ...
    - **A separate case: the safety mechanism prevents the review from proceeding (`available` but safety block)**: when the diff content trips the model's safety classifier and `codex exec review` cannot be completed, that skip is **different in meaning** from the "codex not installed" skip — the gate is **not established**, which is not "codex clean" (no evasive paraphrasing or prompt alteration). The official recovery is **a fresh session** (a continuation spawn inheriting the same worktree and commits re-runs the review; state the inherited HEAD SHA in the completion report. Ask the Lead to restart the session). If recovery is impossible, do **not** report "codex clean"; state "gate not established (safety block, HEAD=`<sha>`)" explicitly and ask for the Lead's judgment
  - **The following applies only if codex is run**:
    - For Blocker / Major findings, add a fix commit before the completion report
    - **The overall round cap is 3 by default** (if the brief's "Implementation guidance" explicitly specifies a different value, that takes precedence). **When you reach the cap, do not autonomously enter round N+1**; report to the Lead and stop, attaching the remaining Blocker / Major findings + a self-assessment (whether this has become a design problem, or is mid-way through a healthy convergence where separate issues surface one by one) (stop at the cap even in cases where a different Major appears every round; the decision to continue is left to the human)
    - **If the same finding (for example: tightening a loose match / narrowing a type) does not clear after 3 rounds, report it immediately as a design problem even before reaching the cap**. The same finding / location recurring after a fix is a sign that the fix approach itself is the problem -- distinguish it from a healthy convergence where separate issues each clear in one round (which may continue up to the cap). Report completion immediately and ask the Lead to decide whether to reduce scope / rethink the design (prevents infinite loops)
  ...
  - Review instruction example (diff self-review): `codex exec review --base origin/main -m gpt-5.5 -c model_reasoning_effort=medium` (pass the branch base, normally `origin/main`, to `--base`. Use the **remote-tracking `origin/main` rather than the local `main`** because a stale local `main` in a shared clone drags another task's diff into the review. The codex built-in review prompt returns Blocker/Major-equivalent findings).
  - **Canonical execution form (diff self-review = review surface)**: `codex exec review --base origin/main -m gpt-5.5 -c model_reasoning_effort=medium < /dev/null` -- run in the **foreground** and read the output (Blocker/Major-equivalent) before moving on. Close stdin explicitly with `< /dev/null`. Avoid background `&` + log redirect because it leads to reporting completion without waiting for or reading the findings, slipping past the gate (the fast review surface naturally gates when you wait in the foreground). When copy-pasting the command, do not leave redirect-character placeholders like `<main>` / `<N>` (they get misinterpreted as `< main` and fail). **Run `git fetch origin` once before referencing `origin/main`** (even if you pulled at the start, refresh the remote-tracking ref right before the review; if the fetch fails, the review itself still continues).
...
- full: run normal verification such as the existing test suite / lint / type-check until green, and after completing the commit, if the codex CLI is available, run a diff self-review with **`codex exec review --base origin/main -m gpt-5.5 -c model_reasoning_effort=medium` (review surface)** (the long-prompt direct form is deprecated; roughly 2x faster on small/medium diffs and at parity on the safety side; do not raise effort on large diffs). For Blocker / Major findings, add a fix commit before the completion report. **The Codex round cap is 3 by default** (if the brief's Implementation guidance specifies a different value, that takes precedence): when you reach the cap, do not autonomously enter round N+1; report to the Lead and stop, attaching the remaining findings + a self-assessment (design problem vs. mid-convergence) (stop at the cap even if a different issue appears every round; the human decides whether to continue). If the same finding does not clear after 3 rounds, report it as a design problem even before the cap (distinguish from a healthy convergence where separate issues each clear in one round). Minor / Nit findings should be left as-is (document them as known limitations). **A skip where codex is available but the safety mechanism prevents the review from proceeding is treated separately from the "codex not installed" skip** (the gate is not established, which is not codex clean; no evasive paraphrasing): official recovery = a fresh session (a continuation spawn inheriting the same worktree and commits re-runs the review; state the inherited HEAD SHA in the completion report); if unrecoverable, do not report "codex clean" — state "gate not established (safety block, HEAD=`<sha>`)" explicitly. **Knowledge layer privacy (applies only in full mode when recording to `knowledge/raw/`)**: `knowledge/raw/` and `knowledge/curated/` are committed to a public OSS repository. Do not write operator-private content such as operator personal names, internal system identifiers, customer data, secrets, or internal URLs. Do not record learnings that include such information; escalate them to the Lead (secretary).
