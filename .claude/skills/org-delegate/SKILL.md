...
- **Before delegating a suspected bug in a pin-managed internal package (`claude-org-runtime`, etc.)**: confirm that the day's [`/org-start`](../org-start/SKILL.md) Block C2 runtime drift check was clean (exit 0). If it detected drift (exit 1) or was "PyPI unconfirmed" (exit 2 = could not reach PyPI, e.g. run inside the sandbox), the old venv may be hitting a bug already fixed upstream, so first upgrade the venv + bump the pin window to bring it up to date, and confirm the issue still reproduces before delegating (prevents a phantom dispatch against an already-fixed bug). The pin lag is a session constant, so one check at org-start suffices; a per-delegation re-check is not needed (background: 2026-07-08 #119).
...
### Pre-written brief wording for parallel delegations that edit the same file

When you dispatch multiple Workers in parallel and you already know at brief-assembly time that they will **edit the same file (especially the same region)**, put wording to the following effect into `gen_delegate_payload.py`'s `--impl-guidance` up front:

> The parallel task {parallel_task_id} is editing the same file {path}. The Lead will send a follow-up on merge order; only after receiving that follow-up, rebase onto the base upstream (normally `origin/main`) and then send your completion report.

Two operational notes:

- **The Lead does not send the follow-up merely because merge order has been decided.** Send it to the following Worker only after confirming that the preceding PR has been merged and is reflected in the base upstream (sending it right after the ordering decision makes the Worker rebase onto an upstream that does not yet contain the preceding change, so the conflict prevention does not hold).
- **For how to read the rebase target and its applicable scope**, follow the "rebase before the completion report" rule in [`.claude/skills/org-delegate/references/worker-claude-template.md`](references/worker-claude-template.md) (`full` only; if the PR base is something other than `origin/main`, read it as the actual base upstream).

An after-the-fact follow-up instruction (sent on via peer message) can also rescue the situation (the real example of the 2026-07-22 ja#747 / ja#748 parallel edits), but putting it in at the brief stage removes one Lead -> Worker round trip and prevents the integration-point conflict in advance. Combined with the existing practice of rebasing onto the base upstream before reporting completion, the Lead's remaining work shrinks to just deciding the merge order and sending the follow-up.
...
- For the minimum ack contents and per-kind example texts, see [`.claude/skills/org-delegate/references/ack-template.md`](references/ack-template.md). **ack ≠ user approval**: `git push` / `gh pr create` / `/pr-watch-pane` are issued only after the user's explicit OK
...
This subsection applies not only to the literal word "progress" but to **all kinds of worker→secretary peer messages other than completion reports (2a) / judgment-escalation・blockers (0) / plan・prep handoff** (intermediate handoff reports such as progress sharing, rebase-completion reports, confirmation requests). plan / prep handoff stays on the existing flow of being journaled under their dedicated kinds (`plan_delivered` / `prep_delivered`, [`docs/journal-events.md`](../../../docs/journal-events.md)) and is not switched to `worker_reported` (the design-flow consumer side references the dedicated kinds).
...
- **Always append `worker_reported` to the DB events table (non-omittable, Issue #699)**:
  ```bash
  bash tools/journal_append.sh worker_reported worker=worker-{task_id} task={task_id} summary="<summary>"
  ```
  Match the payload keys to the event catalog (the `worker_reported` row in [`docs/journal-events.md`](../../../docs/journal-events.md): `worker`, `task`, `summary`). Keep the kind unified as `worker_reported` (do not use aliases such as `worker_progress`). The dispatcher's PANE_OUTPUT_WITHOUT_PEER_MSG judgment ([`.dispatcher/references/worker-monitoring.md`](../../../.dispatcher/references/worker-monitoring.md) Step 5.2) references the `worker_reported` trace in the events table via SQL, so a missing journal entry for an intermediate handoff report creates a state of "the peer message arrived but there is no trace in events," which is a direct cause of a false positive that misdetects a healthy, running worker as a silent dead-lock (real example on 2026-07-08 kura conveyor).
