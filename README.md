...
- Operational tools: `tools/check_renga_compat.py` / `tools/gen_worker_brief.py` / `tools/org_setup_prune.py` / `tools/journal_*` / `tools/pr_watch.*` / `tools/state_migrate.py` / `tools/state_db/` (state DB writer / importer / drift_check / curator_archive; after the live migration in Issue [#267](https://github.com/suisya-systems/claude-org-ja/issues/267), `.state/state.db` is the only source of truth for organization state. `.state/org-state.md` is a derived artifact automatically regenerated by the post-commit processing of `StateWriter.transaction()`, and `.state/journal.jsonl` was removed in M4)
